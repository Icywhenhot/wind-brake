...
public class WindBrakeConfigScreen extends Screen {
    ...
    // 26.x reworked the GUI to a render-state extraction model: the old
    // render(GuiGraphics, ...) is now extractRenderState(GuiGraphicsExtractor, ...).
    @Override
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
        // Don't extract the background here: the framework
        // (Screen#extractRenderStateWithTooltipAndSubtitles) already does it, and its blur may
        // only run once per frame, so doing it again hard-crashes the client. super's
        // extractRenderState draws just the widgets; the title goes on top.
        super.extractRenderState(g, mouseX, mouseY, partialTick);
        // Color is ARGB: 0xFFFFFF has alpha 0 and transparent text is skipped, so the title
        // would be invisible. Use 0xFFFFFFFF (opaque white). drawCenteredString -> centeredText.
